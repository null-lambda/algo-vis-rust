...
#[allow(unused)]
pub mod voronoi {
    use crate::geometry::{Ordered, Point};
    ...
    pub mod graph {
        ...
        pub const UNSET: usize = 1 << 32;
        pub const INF: usize = 1 << 31;
    ...
    pub mod builder {
        mod splay {
            ...
            impl Tree {
                ...
                pub fn insert(&mut self, branch: Branch, value: Breakpoint) {
                    ...
                    side.as_mut().map(|mut side| unsafe {
                        Node::link_sides(branch, side.as_mut(), new_node.as_mut());
                    });
                ...
                pub fn pop_root(&mut self) -> Option<Box<Node>> {
                    ...
                    unsafe {
                        match (left.as_mut(), right.as_mut()) {
                            ...
                            (Some(_), None) => {
                                root.side[Left as usize].unwrap().as_mut().side[Right as usize] =
                                    None;
                                self.root = left;
                            }
                            (None, Some(rii_)) => {
                                root.side[Right as usize].unwrap().as_mut().side[Left as usize] =
                                    None;
                                self.root = right;
                            }
                            (None, None) => {}
        ...
        impl Builder {
            ...
            pub fn step(&mut self) -> bool {
                ...
                let Some((Reverse(y), Trivial(event))) = self.events.pop() else {
                    return false;
                };
                match event {
